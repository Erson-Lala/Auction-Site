...
export default function Bid() {
     ...
     const handleEnterBid = async () => {
          setLoader(true)
          if(userName.userId == auctionItemData.sellerID){
               toast.error("You cannot did because you are Seller")
               setLoader(false)
               return
          }
          if (userName.userId !== auctionItemData.topBidUserId) {
               ...
               var tempWallet = parseFloat(userData.wallet) + parseFloat(auctionItemData.topBid)
               var wallet     = tempWallet - parseFloat(newBid)
               if (parseFloat(auctionItemData.topBid) >= parseFloat(newBid)) {
                    toast.error("Your bid is less than top bid ")
                    setLoader(false)
                    return
               ...
               try {
                    ...
                    navigate("/homes")
                    setTimeout(() => {
                         toast.success(`Your remaining balance is: ${wallet}`)
                         return
                    }, 1000)
               } catch (e) {
                    toast.error("Server encounter an unexception condition")
